...
class Express_Tracking(object):
    ...
    def __WelCome(self):
        # 打印程序描述
        tb = pt.PrettyTable()
        tb.field_names = ['名称', '快递批量查询截图工具（非顺丰单号）']
        tb.add_row(['作者', '王文铖'])
    ...
    def get_route_info(self):
        ...
        self.screenshot_img_temp = self.mypath[4] + r'\%s-WLJD-%s.png' % (sself.shot_name, self.bill_number)
        try:
    ...
    def get_dzcg_info(self):
        ...
        self.screenshot_img2 = self.mypath[1] + r'\%s-YDXQ-%s.png' % (self.num, self.bill_number)
        self.screenshot_img3 = self.mypath[1] + r'\%s-DZCG-%s.png' % (self.num, self.bill_number)
        self.dzcg = ''
        try:
    ...
    def sf_single(self):
        ...
        if verify_times > 4:
            self.logger.info('验证解析尝试5次失败，可能不是顺丰单号，不再尝试，开始下一个单号')
        if verify_ok==True:
    ...
    def kd100_single(self):
        ...
        if notFindTip=='display: none;':
            self.screenshot_img4=self.mypath[1] + r'\%s-WLJD-%s.png' % (self.shot_name, self.bill_number)
...
class Input_and_Output(object):
    ...
    def load_input(self):
        try:
            ...
            print(e,'快递单号读取错误，检查是否存在 bill_number.xlsx 工作簿') 
...
def main():
    ...
    if count>0:
        ...
        io.csv_to_xlsx()
        if count/30==0:
            track.logger.info('每查询30个，休息2分钟')
            time.sleep(120)
    else:
        pass
